...
// Categories that are always bills regardless of amount
const BILL_CATEGORIES = /mortgage|rent|utilities|insurance|student loans|childcare|phone|internet|cable|telecom|auto loan|car loan|loan payment|medical|gym|fitness|membership/i;

// Noise to always exclude from recurring detection
const NEVER_RECURRING = /transfer|cash, checks|atm|venmo|zelle|paypal|apple cash/i;

// Shopping/grocery merchants where repeated amounts are coincidence not subscription
const COINCIDENCE_CATS = /shopping|groceries|dining|restaurants|travel|gas/i;
...
export async function getRecurring() {
  ...
  const addGroup = (txs, type) => {
    ...
    const key = `${txs[0].merchant}|${median.toFixed(2)}`;
  ...
  for (const txs of Object.values(incomeGroups)) {
    // Split mixed-magnitude groups (e.g. weekly paycheck vs mid-month commission).
  ...
  const expenses = transactions.filter((t) => (t.amount ?? 0) < 0 && !NEVER_RECURRING.test(t.category ?? '') && !NEVER_RECURRING.test(t.merchant ?? ''));

  // ── Bills: known bill categories, any regularity ───────────────────────────
  const billGroups = {};
  for (const t of expenses) {
    if (!BILL_CATEGORIES.test(t.category ?? '') && !BILL_CATEGORIES.test(t.merchant ?? '')) continue;
    (billGroups[t.merchant] ??= []).push(t);
  }
  Object.values(billGroups).forEach((txs) => addGroup(txs, 'bill'));

  // ── Subscriptions: same merchant + same amount, appearing 2+ times ─────────
  // Catches anything not already classed as a bill. Excludes coincidence cats
  // (shopping/dining/groceries) where repeated amounts don't mean subscription.
  const subGroups = {};
  for (const t of expenses) {
    if (BILL_CATEGORIES.test(t.category ?? '') || BILL_CATEGORIES.test(t.merchant ?? '')) continue;
    if (COINCIDENCE_CATS.test(t.category ?? '')) continue;
    const amt = Math.abs(t.amount);
    (subGroups[`${t.merchant}|${amt.toFixed(2)}`] ??= []).push(t);
  }
  for (const txs of Object.values(subGroups)) {
    if (txs.length < 2) continue; // 2+ identical hits = recurring
    addGroup(txs, 'subscription');
  }

  // ── Catch-all: any non-bill merchant that appears 3+ times regularly ───────
  // Picks up things like pet insurance, professional services, etc. that don't
  // match exact category names but clearly recur.
  const catchGroups = {};
  for (const t of expenses) {
    if (BILL_CATEGORIES.test(t.category ?? '') || COINCIDENCE_CATS.test(t.category ?? '')) continue;
    (catchGroups[t.merchant] ??= []).push(t);
  }
  for (const txs of Object.values(catchGroups)) {
